...
import com.`fun`.hairclipper.databinding.ActivityMachine8Binding

class Machine8 : BaseClass() {
    var vibrator: Vibrator? = null
    var mediaPlayer: MediaPlayer? = null
    private val binding by lazy { ActivityMachine8Binding.inflate(layoutInflater) }
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(binding.root)
        loadBannerAd()
        binding.homeBtn.setOnClickListener { handleBackPressed() }
        mediaPlayer = MediaPlayer.create(this@Machine8, R.raw.hair_clipper8)
    ...
    private fun loadBannerAd() {
        val adId = if (AdConstants.TEST_ADS) {
            ...
            RemoteConfig.getString(RemoteConfig.MACHINE8_BANNER_AD_ID)
        }
        val makeCollapsible = RemoteConfig.getBoolean(RemoteConfig.MACHINE8_BANNER_MAKE_COLLAPSIBLE)
        val enable = RemoteConfig.getBoolean(RemoteConfig.ENABLE_MACHINE8_BANNER_AD)
